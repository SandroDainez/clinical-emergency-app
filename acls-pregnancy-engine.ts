/**
 * Engine mínimo para o módulo "PCR na Gestação".
 * Tela de referência estática; engine apenas expõe ClinicalEngine para roteamento.
 *
 * Mesmo padrão dos demais satélites do ACLS (ritmos, farmacologia, pós-PCR, causas
 * reversíveis): o conteúdo vive na tela, e a máquina de estados do ACLS não é
 * tocada. Acrescentar um ramo de gestação ao reducer da PCR mexeria no núcleo mais
 * testado do app para entregar conteúdo que é, na prática, consulta rápida.
 */
...
const PROTOCOL_ID = "pcr_gestacao_acls";

const STATIC_STATE: ProtocolState = {
  type: "action",
  text: "Referência — PCR na Gestação",
};
...
function getCurrentStateId(): string { return "pcr_gestacao_acls_inicio"; }
...
function getEncounterSummary(): EncounterSummary {
  ...
  return {
    ...
    currentStateId: "pcr_gestacao_acls_inicio",
    currentStateText: "Referência — PCR na Gestação",
    shockCount: 0,
...
function getEncounterSummaryText(): string { return "Módulo de referência estática — PCR na Gestação"; }
