/**
 * Engine mínimo para o módulo "Engasgo (OVACE) no Adulto".
 * Tela de referência estática; engine apenas expõe ClinicalEngine para roteamento.
 *
 * Mesmo padrão dos demais satélites do ACLS (ritmos, farmacologia, pós-PCR,
 * causas reversíveis, PCR na gestação): o conteúdo vive na tela, e a máquina de
 * estados da PCR não é tocada.
 */
...
const PROTOCOL_ID = "ovace_adulto";

const STATIC_STATE: ProtocolState = {
  type: "action",
  text: "Referência — Engasgo (OVACE) no Adulto",
};
...
function getCurrentStateId(): string { return "ovace_adulto_inicio"; }
...
function getEncounterSummary(): EncounterSummary {
  ...
  return {
    ...
    currentStateId: "ovace_adulto_inicio",
    currentStateText: "Referência — Engasgo (OVACE) no Adulto",
    shockCount: 0,
...
function getEncounterSummaryText(): string { return "Módulo de referência estática — Engasgo (OVACE) no Adulto"; }
