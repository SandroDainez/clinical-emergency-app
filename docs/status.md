...
| 34b | achado | baixa · não medido | **AC-65**: a trava de procedência cobre dez termos em toda superfície desde a 16ª rodada (§7.21), mas não "§" isolado. Códigos "F-nn" fora do portão da IVT não foram varridos. | a medir | código |
...
| entrega | escopo | estado |
|---|---|---|
| **Entrega 1 · procedência** | «Fonte:», «Conclusão:», «a localizar», «transcrit», «slot» fora do texto visível (plano até 48 h, caminho hemorrágico, Destino, anti-hipertensivos); trava e2e pelas abas que a barra mostra em nove cenários, com visita por gesto às superfícies fora da barra; prova estática em todo `.tsx` do AVC | ✅ `e680ec0` |
| **Entrega 2 · AC-95/98/91/92** | catálogo e caminho com a mesma validação (COR+LOE+população → formal; o resto pendente nos dois, reversões por agente incluídas); «Centro de referência recusou»; pendência de sedação não registrada; «Nada por via oral» pela TC com resultado | ✅ `e680ec0` |
| **Entrega 3 · textos** | plural por contagem PT/ES; «retida»; pendências do caminho sem prefixo | ✅ `e680ec0` |

`test:all` no HEAD `e680ec0`: ✅ **EXIT=0** · 149 `npm run` + 1 `node` · Playwright **617 passed** (9,8 min), sem failed ou skipped · prova rodada 16 27/27 · e2e rodada 16 6/6 · procedência 9/9 · críticos 183/183 · 141 travas ligadas · índice 129 declaradas · push `77e4a88..e680ec0`; `git ls-remote` = `e680ec0d89a8`; divergência 0/0. Capturas a 375 px enviadas ao autor (21).

**Rodada anterior (15ª), concluída · capturas antes da entrega, AC-81/82/83/85/88, agenda viva, caminho hemorrágico (A07):** detalhe em `docs/avc/auditoria/7.20-rodada-15.md`.

| entrega | escopo | estado |
|---|---|---|
| **Capturas da 14ª rodada** | quatro defeitos que os testes verdes não pegaram (minutos crus, agenda parada, registros longe das tarefas, origem repetida) | ✅ `ebc736f` |
| **AC-81/82/83** | «(tipo não determinado)»; pendência para recuperar o basal; sem sedação = exame válido, só o item 10 UN, lembrete NIH no 1b | ✅ `ebc736f` |
| **AC-85 · AC-88** | desfecho negativo de IVT e EVT com decisão global; resultado das transversais e «Nada por via oral» no cabeçalho | ✅ `ebc736f` |
| **Caminho hemorrágico (A07)** | abas reduzidas, título próprio, bloqueio com motivo, tipo, anticoagulante do Paciente, marcos da neurocirurgia, interrupção da infusão pelo gesto | ✅ `ebc736f` · AC-95 decidido, implementação na 16ª rodada |

`test:all` no HEAD `e775061`: ✅ **EXIT=0** · 148 `npm run` + 1 `node` · Playwright **606 passed** (9,5 min), sem failed ou skipped · rodada 15 60/60 · críticos 183/183 · 140 travas ligadas · índice 128 declaradas · push `29a5f3c..e775061`; `git ls-remote` = `e775061a6a57`; divergência 0/0. As duas execuções anteriores (`ebc736f`, `fe95af0`) falharam e não foram enviadas (§7.20).

**Rodada anterior (14ª), concluída · via aérea avançada, NIHSS sob sedação como contexto, A04 com dois caminhos, leve ≠ incapacitante, plano até 48 h:** detalhe em `docs/avc/auditoria/7.19-rodada-14.md`.
...
| entrega | escopo | estado |
|---|---|---|
| **Contrato de navegação (C05)** | `encounterId`, origem com rolagem, pilha na trilha, retorno com eventos/suporte/resposta/pendências; via aérea, ventilação e sedoanalgesia "indisponível neste app"; nenhum botão que simule execução | ✅ `16b92ac` · fecha AC-16 |
| **Via aérea como conduta externa (A09)** | seis campos sem fármaco/dose/parâmetro; suporte ativo no topo; eixo A "intervenção registrada · reavaliação pendente"; NIHSS basal preservado e marcado; item 10 sugerido; «não sei» mantém pendência | ✅ `16b92ac` · AC-76/AC-77 a confirmar |
| **Cronologia (A04)** | «Sem essa informação» por marco (chegada incluída); caminho de início desconhecido a partir da recomendação existente | ✅ `16b92ac` |

`test:all` no HEAD `16b92ac`: ✅ **EXIT=0** · 146 `npm run` + 1 `node` · Playwright **585 passed** (9,1 min), sem failed ou skipped · rodada 13 40/40 · críticos 183/183 · 138 travas ligadas · índice 126 declaradas · push `38e1acd..16b92ac`; `git ls-remote` = `16b92acb3067`; divergência 0/0.

**Rodada anterior (12ª), concluída · auditoria append-only, AC-71, AC-72, AC-73, nome de exame; contrato de navegação bloqueado:** detalhe em `docs/avc/auditoria/7.17-rodada-12.md`.

| entrega | escopo | estado |
|---|---|---|
| **Entrega 0 · auditoria append-only** | 24 arquivos em `docs/avc/auditoria/` (1.446 linhas antes e depois, byte a byte); índice gerado; prova append-only no `test:all` e antes de todo push | ✅ `844b852` · prova 2·5 → 12/12 |
| **AC-71** | estimativa aceita horário futuro; observado mantém o teto | ✅ `bbbf109` |
| **AC-72** | teleconsulta por marcos; parecer exige texto e autor | ✅ `bbbf109` |
| **AC-73** | «Registrado por engano» sempre no evento de piora da linha do tempo | ✅ `bbbf109` |
| **Nome de exame** | título da linha de prioridade da imagem sem truncar | ✅ `bbbf109` |
| **Entrega 2 · contrato de navegação (C05)** | AVC → via aérea → pós-intubação → retorno | ⛔ **não iniciada**: os módulos-alvo não existem; decisão A/B/C na fila (linha 19) |

`test:all` no HEAD `bbbf109`: ✅ **EXIT=0** · 145 `npm run` + 1 `node` · Playwright **578 passed** (8,9 min), sem failed ou skipped · rodada 12 18/18 · append-only 12/12 · críticos 183/183 · 137 travas ligadas · índice 125 declaradas · push `1e27700..bbbf109`; `git ls-remote` = `bbbf10939`; divergência 0/0.

**Rodada anterior (11ª), concluída · «Preciso de ajuda» (fecha AC-10), AC-67, AC-68, AC-69, eixos reabertos e porta do e2e:** detalhe em `docs/avc/auditoria/7.16-rodada-11.md`.
