...
**Branch:** `refactor/clinical-modules-rebuild` · **HEAD de código:** `bbbf109`, enviado; por cima, um commit só de `docs/` (D-PEND-17)

## Fila consolidada · tudo que está aberto (2026-09-13, atualizada na 12ª rodada)

Fonte de cada linha:
- **AC-\*:** `docs/avc/auditoria-vs-spec.md` (índice gerado; seções append-only em `docs/avc/auditoria/`, §1 e §7.4–§7.17).
...
| 19 | decisão | alta | **AC-16 / A09 · contrato de navegação (C05) bloqueado**: os módulos-alvo (via aérea/ISR, ventilação mecânica, sedoanalgesia) **não existem no app** desde 2026-08-27; sobrou `protocols/isr_rapida.json` sem engine (§7.17). Opções: **(A)** contrato completo com os três destinos "indisponível" + registro de conduta externa; **(B)** restaurar o módulo de ISR do JSON legado com o estado de validação que ele tem; **(C)** adiar. Nada da Entrega 2 foi escrito. | aguardando | autor |
...
| 26 | achado | média | **AC-16**: sem chamada nem retorno entre módulos; intubação e sedação ausentes (A09). | bloqueado pela linha 19 | autor, depois código |
...
**Rodada mais recente (2026-09-13, 12ª) · auditoria append-only, AC-71, AC-72, AC-73, nome de exame; contrato de navegação bloqueado:** detalhe em `docs/avc/auditoria/7.17-rodada-12.md`.
