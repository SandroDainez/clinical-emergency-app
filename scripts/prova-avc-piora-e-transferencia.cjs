...
if (D !== undefined) {
  ...
  const evento = piorou.fatos[piorou.fatos.length - 1];
  ...
  conf("A11 · a trilha anterior fica intacta (histórico preservado)",
    JSON.stringify(piorou.fatos.slice(0, tratado.fatos.length)) === JSON.stringify(tratado.fatos) && piorou.fatos.length === tratado.fatos.length + 1,
...
if (T !== undefined) {
  ...
  let tr = reg(vazio(), "transf_estado", "Solicitada");
  tr = reg(tr, "transf_estado", "Contato realizado");
  ...
  const semAceite = reg(tr, "transf_estado", "Transporte confirmado");
  ...
  const comAceite = reg(reg(tr, "transf_estado", "Aceite"), "transf_estado", "Transporte confirmado");
  ...
  /* ── recusa registrada ── */
  const recusa = reg(tr, "transf_estado", "Recusa");
  ...
  conf("A12 · nova solicitação depois da recusa → a tarefa sai", !T.pendenciasDoDestino(reg(recusaMotivo, "transf_estado", "Solicitada")).some((p) => p.id === "revisar_acesso_transferencia"), "⛔");
  ...
  const impedidaEmTransf = reg(reg(impedida, "transf_estado", "Solicitada"), "transf_estado", "Aceite");
  ...
  if (D !== undefined) {
    const espera = reg(reg(vazio(), "transf_estado", "Solicitada"), "transf_estado", "Aceite");
  ...
  const errado = reg(vazio(), "transf_estado", "Aceite");
  const corrigido = E.desfazerRegistro(errado, "transf_estado", rel);
