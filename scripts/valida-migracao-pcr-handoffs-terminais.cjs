...
exige(tachyNode, 'disposition: "icu"', "taquicardia sem pulso já parece migrada; revisar/remover esta trava de preparação");
exige(bradyNode, 'disposition: "icu"', "bradicardia sem pulso já parece migrada; revisar/remover esta trava de preparação");
...
// Contratos de contexto existem e as dívidas apontam para eles.
for (const token of [
...
if (transitions.includes('id: "taquicardia-sem-pulso-pcr"') || transitions.includes('id: "bradicardia-sem-pulso-pcr"')) {
  erros.push("registry vivo já contém os handoffs PCR; a migração preparada não está mais no estado pré-execução");
}
...
console.log("\n✅ migração taquicardia/bradicardia → PCR está preparada, com contexto explícito, e ainda não foi executada.\n");
