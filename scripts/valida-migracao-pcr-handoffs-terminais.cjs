...
if (migrada) {
  const pos = path.join(raiz, "scripts/valida-pcr-handoffs-terminais-pos-migracao.cjs");
  const r = spawnSync(process.execPath, [pos], { cwd: raiz, encoding: "utf8" });
  if (r.stdout) process.stdout.write(r.stdout);
  if (r.stderr) process.stderr.write(r.stderr);
  if (r.status !== 0) process.exit(r.status ?? 1);
  console.log("✅ Migração terminal PCR já executada; estado pós-migração validado.");
  process.exit(0);
}
...
exige(tachyNode, 'disposition: "icu"', "taquicardia sem pulso não está no estado pré-migração esperado");
exige(bradyNode, 'disposition: "icu"', "bradicardia sem pulso não está no estado pré-migração esperado");
...
console.log("✅ Migração terminal PCR ainda não executada; estado pré-migração consistente.");
