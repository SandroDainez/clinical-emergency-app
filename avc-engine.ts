...
function buildFields(snapshot: AvcCaseSnapshot): AuxiliaryPanelField[] {
  const fields: AuxiliaryPanelField[] = [
    ...
    field("Comorbidades relevantes", "comorbidities", session.assessment.comorbidities, "História clínica relevante", { fullWidth: true }),
    field("Antiagregantes / anticoagulantes", "antithrombotics", session.assessment.antithrombotics, "História clínica relevante", { fullWidth: true }),
    field("Função renal / observação", "renalFunction", session.assessment.renalFunction, "História clínica relevante", { fullWidth: true }),
