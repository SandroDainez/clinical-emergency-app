/**
 * Descrições dos cards do hub (clinical-modules.ts) — dicionário PT → ES.
 * Os títulos já vivem nos dicionários do ACLS/interface; aqui ficam só as
 * descrições, que apareciam em português mesmo com o app em espanhol.
 */
export const ES_HUB_DESCRICOES: Record<string, string> = {
  // ── Descritores curtos da UI 2.0 (substituíram as descrições longas) ──
  "Atropina, marca-passo": "Atropina, marcapasos",
  "QRS estreito × largo": "QRS estrecho × ancho",
  "5 Hs e 5 Ts": "5 H y 5 T",
  "Deslocamento uterino": "Desplazamiento uterino",
  "Golpes e compressões": "Golpes y compresiones",
  "Metas e neuroproteção": "Metas y neuroprotección",
  "Chocável × não chocável": "Desfibrilable × no desfibrilable",
  "Dose e indicação": "Dosis e indicación",

  "Bundle inicial de sepse do adulto com decisões clínicas, fluidos, antimicrobianos e vasopressor.":
    "Paquete inicial de la sepsis del adulto con decisiones clínicas, líquidos, antimicrobianos y vasopresor.",

  "Cálculo prático de preparo e taxa para noradrenalina, adrenalina, vasopressina, dopamina e dobutamina.":
    "Cálculo práctico de preparación y velocidad para noradrenalina, adrenalina, vasopresina, dopamina y dobutamina.",

  "Calculadoras práticas para distúrbios de sódio, potássio, cálcio, magnésio, fósforo e cloro com preparo e velocidade.":
    "Calculadoras prácticas para los trastornos de sodio, potasio, calcio, magnesio, fósforo y cloro, con preparación y velocidad.",

  "Intubação em sequência rápida: indicações, drogas, equipamento e passo a passo.":
    "Intubación de secuencia rápida: indicaciones, fármacos, equipamiento y paso a paso.",

  "Roteiro resumido: clínica, tratamento imediato e destino — ciclo curto.":
    "Guion resumido: clínica, tratamiento inmediato y destino — ciclo corto.",

  "Cetoacidose diabética vs estado hiperosmolar: classificação, volume, insulina, potássio e monitorização.":
    "Cetoacidosis diabética vs. estado hiperosmolar: clasificación, volumen, insulina, potasio y monitorización.",

  "Cenário clínico, peso e parâmetros atuais; metas de Vt/PEEP e passo a passo no ventilador em linguagem simples.":
    "Escenario clínico, peso y parámetros actuales; objetivos de volumen corriente/PEEP y paso a paso en el ventilador, en lenguaje sencillo.",

  "Exposição, manifestações, choque; dose de adrenalina IM por peso e passo a passo terapêutico.":
    "Exposición, manifestaciones, choque; dosis de adrenalina IM por peso y paso a paso terapéutico.",

  "Fluxo de AVC isquêmico e hemorrágico com tempos críticos, NIHSS, imagem, reperfusão, prescrição e destino.":
    "Flujo del ACV isquémico y hemorrágico con tiempos críticos, NIHSS, imagen, reperfusión, prescripción y destino.",

  "Dor torácica, STEMI, NSTEMI, angina instável e angina estável com ECG, troponina, scores, reperfusão, medicações e destino.":
    "Dolor torácico, IAMCEST, IAMSEST, angina inestable y angina estable con ECG, troponina, escalas, reperfusión, medicación y destino.",

  "TEP do diagnóstico à reperfusão: estabilidade, Wells, D-dímero/AngioTC, estratificação de risco (VD + biomarcadores + sPESI), anticoagulação e trombólise.":
    "TEP del diagnóstico a la reperfusión: estabilidad, Wells, dímero D/angiotomografía, estratificación de riesgo (ventrículo derecho + biomarcadores + sPESI), anticoagulación y trombólisis.",

  "Emergência hipertensiva da gestação: convulsão, sulfato de magnésio (Pritchard/Zuspan) com tríade de segurança, crise hipertensiva, momento do parto e pós-parto.":
    "Emergencia hipertensiva del embarazo: convulsión, sulfato de magnesio (Pritchard/Zuspan) con tríada de seguridad, crisis hipertensiva, momento del parto y posparto.",

  "Calculadora de sedativos, opioides e bloqueadores neuromusculares: diluição, concentração, dose e taxa (mL/h) ou bolus, com faixas por RASS e alertas de segurança.":
    "Calculadora de sedantes, opioides y bloqueantes neuromusculares: dilución, concentración, dosis y velocidad (mL/h) o bolo, con rangos por RASS y alertas de seguridad.",

  "Escores e calculadoras à beira-leito: peso predito (VM), clearance/TFG, osmolalidade, ânion gap, Glasgow, qSOFA, SOFA, Wells (TEP), CURB-65 e HEART — com interpretação e fonte.":
    "Escalas y calculadoras a pie de cama: peso predicho (VM), aclaramiento/TFG, osmolalidad, brecha aniónica, Glasgow, qSOFA, SOFA, Wells (TEP), CURB-65 y HEART — con interpretación y fuente.",

  "Atendimento inicial ao traumatizado grave (ATLS): controle da hemorragia exsanguinante, XABCDE, reanimação hemostática 1:1:1, ácido tranexâmico e damage control.":
    "Atención inicial al politraumatizado grave (ATLS): control de la hemorragia exanguinante, XABCDE, reanimación hemostática 1:1:1, ácido tranexámico y cirugía de control de daños.",

  "Classificação por Glasgow, indicação de tomografia, prevenção da lesão secundária, reversão de anticoagulação e controle da hipertensão intracraniana.":
    "Clasificación por Glasgow, indicación de tomografía, prevención de la lesión secundaria, reversión de la anticoagulación y control de la hipertensión intracraneal.",

  "Protocolo por tempo: benzodiazepínico em dose plena, antiepiléptico IV de 2ª linha e anestésico com IOT e EEG no mal epiléptico refratário, com doses por peso.":
    "Guía por tiempo: benzodiacepina a dosis plena, antiepiléptico IV de 2.ª línea y anestésico con intubación y EEG en el estado epiléptico refractario, con dosis por peso.",

  "Síndromes tóxicas (toxidromes), descontaminação, antídotos específicos por tóxico com dose e via, e indicações de hemodiálise.":
    "Síndromes tóxicos (toxíndromes), descontaminación, antídotos específicos por tóxico con dosis y vía, e indicaciones de hemodiálisis.",

  "Reconhecimento e diferencial do choque — hipovolêmico, obstrutivo, cardiogênico e distributivo — com mecanismo, sinais confirmatórios e conduta imediata.":
    "Reconocimiento y diferencial del choque — hipovolémico, obstructivo, cardiogénico y distributivo — con mecanismo, signos confirmatorios y conducta inmediata.",

  "Diferencial da insuficiência respiratória aguda com exames prioritários, tratamento imediato e critérios de intubação.":
    "Diferencial de la insuficiencia respiratoria aguda con exámenes prioritarios, tratamiento inmediato y criterios de intubación.",

  "Exclusão de catástrofes abdominais, classificação do padrão (inflamatório, obstrutivo, perfurativo, vascular) e definição do destino cirúrgico.":
    "Exclusión de catástrofes abdominales, clasificación del patrón (inflamatorio, obstructivo, perforativo, vascular) y definición del destino quirúrgico.",
};
