/**
 * acls-assistant — assistente operacional de ACLS.
 *
 * ── ⚠️⚠️ ORIGEM DESTE ARQUIVO ────────────────────────────────────────────────
 *
 * ⚠️ Ele ⛔ **não** foi escrito de memória: é o **TypeScript original**, recuperado
 * do `sourcesContent` do bundle ESZIP implantado (versão 6, lido pela Management
 * API em 2026-08-30). O corpo abaixo está **verbatim**, com **⛔ uma única
 * adição** — o bloco de autenticação, marcado no lugar.
 *
 * ── ⛔ O DEFEITO QUE ISTO FECHA (P1, auditoria de 2026-08-30) ────────────────
 *
 * A função está implantada com `verify_jwt = false` e o handler ⛔ **não valida
 * ⛔ nada**: ia de `req.json()` direto à OpenAI, usando a `OPENAI_API_KEY` do
 * servidor, com `Access-Control-Allow-Origin: *`.
 *
 * ⚠️⚠️ Qualquer pessoa que soubesse a URL queimava a cota. ⛔ E ⛔ não havia sequer
 * um `Authorization` sendo lido.
 *
 * ── ⚠️ POR QUE `verify_jwt = true` ⛔ NÃO BASTA SOZINHO ───────────────────────
 *
 * `verify_jwt` valida a **assinatura** do token — e a chave publishable `anon`
 * **é** um JWT válido, que viaja no bundle do cliente. ⛔ Sozinho, ele deixaria
 * passar exatamente quem se queria barrar.
 *
 * ⚠️ Por isso a guarda é dupla: `verify_jwt = true` no `config.toml` **e**
 * `auth.getUser(token)` no handler, exigindo **usuário real**. ⚠️⚠️ E ela roda
 * **antes** de ler a chave da OpenAI e antes de qualquer `fetch` ao provedor —
 * requisição ⛔ não autenticada ⛔ nunca chega a consumir cota.
 *
...
Deno.serve(async (req) => {
  ...
  // ─── ⚠️⚠️ AUTENTICAÇÃO — ÚNICA ADIÇÃO AO FONTE RECUPERADO ─────────────────
  //
  // ⚠️ Falha fechada, e **antes** de tudo: antes de ler a chave da OpenAI,
  // antes de ler o corpo, antes de qualquer `fetch` ao provedor. ⛔ Requisição
  // ⛔ não autenticada ⛔ nunca chega a consumir cota.
  //
  // ⛔⛔ E ⛔ não basta ter um JWT: a chave `anon` **é** um JWT válido e viaja no
  // bundle. `getUser` exige **usuário real** por trás do token.
  ...
  const { data: usuario } = await auth.auth.getUser(token);
  if (!usuario?.user?.id) {
    return jsonResponse({ ok: false, error: "unauthorized" }, 401);
  }
  // ─── fim da adição ────────────────────────────────────────────────────────
